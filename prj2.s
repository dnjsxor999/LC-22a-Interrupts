...
toaster_handler:
        ...
        addi    $sp, $sp, -1                       !Stack +1
        sw      $k0, 0($sp)                        !save prev $k0 on the stack
...
Skip:
        lw $t0, 2($sp)                          !load $t0 register from the stack (SEE THE OFFSET)
        lw $t1, 1($sp)                          !load $t1 register from the stack (SEE THE OFFSET)
        lw $t2, 0($sp)                          !load $t2 register from the stack (SEE THE OFFSET)
        addi $sp, $sp, 3
        di
        lw $k0, 0($sp)                          !load $k0 from stack
        addi $sp, $sp, 1                        !Stack -1
        reti
